...
public class DeltaSolver2 extends LocalSearchSolver{
    private List<DeltaMove2> moveList;
    ...
    public DeltaSolver2(int[][] distanceMatrix, int[][] objectiveMatrix, int[] costs, List<Node> nodes) {
    ...
    private DeltaMove2 Move2DeltaMove(Move move, Solution currentSolution){
        if (move.getType().equals("Intra") &&
            ...
            Edge newEdge1 = new Edge(startNodeA, startNodeB);
            Edge newEdge2 = new Edge(endNodeA, endNodeB);
            ...
            Edge newEdge1 = new Edge(newNodeID, successorID);
            Edge newEdge2 = new Edge(predecessorID, newNodeID);
    ...
    private void rebuildNeighborhood(Solution currentSolution,
        ...
        for (Move move : allMoves) {
            DeltaMove2 deltaMove = Move2DeltaMove(move, currentSolution);
        ...
        moveList.sort(Comparator.comparingInt(DeltaMove2::getDelta));
    }
     ...
     */
    private MoveValidityStatus checkMoveValidity(DeltaMove2 move, Solution currentSolution, Set<Integer> nonSelectedNodes) {
        int[] path = currentSolution.getPath();
     ...
     */
    private Set<Integer> getAffectedPositionsIntra(DeltaMove2 move, int[] pathBeforeMove) {
     ...
     * @return indices inside pathBeforeMove that must be refreshed
     */
    private Set<Integer> getAffectedPositionsInter(DeltaMove2 move, int[] pathBeforeMove) {
        Set<Integer> affectedPos = new HashSet<>();
     ...
     */
    private Set<Integer> getAffectedNodes(DeltaMove2 move, int[] pathBeforeMove) {
    ...
    private void generateNewMoves(Solution currentSolution,
        ...
        if (isInterMove) {
            ...
            for (int affectedNode : affectedNodes) {
                ...
                for (int outsideNode : nonSelectedNodes) {
                    ...
                    Edge newEdge1 = new Edge(outsideNode, successorID);
                    Edge newEdge2 = new Edge(predecessorID, outsideNode);

                    int delta = deltaNodeSwap(affectedNode, outsideNode, currentSolution);

                    if (delta < 0) {
                        DeltaMove2 deltaMove = new DeltaMove2(
                                "Inter",
                                "-",
                                affectedNode,
                                outsideNode,
        ...
        for (int nodeA : affectedNodes) {
            ...
            for (int posB = 0; posB < n; posB++) {
                ...
                Edge newEdge2 = new Edge(oldEdge1.getEndNodeID(), oldEdge2.getEndNodeID());
    ...
    public Solution steepestLocalSearch(Solution currentSolution, Set<Integer> allNodeIDs) {
        ...
        while (!moveList.isEmpty()) {
            ...
            for (int i = 0; i < moveList.size(); i++) {
                DeltaMove2 move = moveList.get(i);
                ...
                if (status == MoveValidityStatus.RECALCULATE) {
                    ...
                    DeltaMove2 updatedMove = Move2DeltaMove(tempMove, currentSolution);
